...
%TWOLAYERMODEL Calculate film-surface temperature and displacement responses.
% Inputs:
...
%% Validate and orient model inputs
    ...
    if numel(p) ~= 3 || any(~isfinite(p)) || any(p <= 0)
        error("TGS:Parameters", ...
            "p must contain positive finite [alpha_f, alpha_s, R].");
    end
    if any(~isfinite(t))
...
%% Calculate material and coupling constants
    ...
    filmThermoelasticCoupling = ...
        S.alphathf*(1+S.nuf)/(1-S.nuf);
    substrateThermoelasticCoupling = ...
        S.alphaths*(1+S.nus)/(1-S.nus);
...
%% Assemble the elastic boundary-condition system
    ...
    elasticMatrix = [ ...
        ...
        1, q*filmThickness-3+4*S.nuf, -exp2qL, ...
            exp2qL*(4*S.nuf-q*filmThickness-3), -1, ...
            3-4*S.nus-q*filmThickness; ...
        1, q*filmThickness, exp2qL, q*filmThickness*exp2qL, ...
            -1, -q*filmThickness; ...
        1, q*filmThickness-2+2*S.nuf, exp2qL, ...
            exp2qL*(q*filmThickness+2-2*S.nuf), -shearRatio, ...
            (2-2*S.nus-q*filmThickness)*shearRatio; ...
        1, q*filmThickness-1+2*S.nuf, -exp2qL, ...
            exp2qL*(2*S.nuf-q*filmThickness-1), -shearRatio, ...
            (1-2*S.nus-q*filmThickness)*shearRatio];
    elasticSolver = decomposition(elasticMatrix,"lu");
...
%% Solve the coupled response at each positive frequency
    for frequencyIndex = 1:positiveCount
        ...
        conductivityRatio = substrateConductivity*beta_s/ ...
            (filmConductivity*beta_f);
        interfaceFactor = 1-conductivityRatio+ ...
            substrateConductivity*beta_s*R;
        thermalDenominator = 2*conductivityRatio+ ...
            interfaceFactor*(1-exp(-2*beta_f*filmThickness));
        ...
        A_f_interface = commonFactor* ...
            (2*conductivityRatio+interfaceFactor)*interfaceDecay;
        B_f_interface = commonFactor*interfaceFactor*interfaceDecay;
        A_s_interface = 2*commonFactor*interfaceDecay;

        elasticForcing = [ ...
            filmThermoelasticCoupling*beta_f*(A_f-B_f)/r_f; ...
            filmThermoelasticCoupling*q*(A_f+B_f)/r_f; ...
            filmThermoelasticCoupling*q*(A_f_interface+B_f_interface)/r_f- ...
                substrateThermoelasticCoupling*q*A_s_interface/r_s; ...
            filmThermoelasticCoupling*beta_f*(A_f_interface-B_f_interface)/r_f- ...
                substrateThermoelasticCoupling*beta_s*A_s_interface/r_s; ...
            filmThermoelasticCoupling*beta_f*(A_f_interface-B_f_interface)/r_f- ...
                shearRatio*substrateThermoelasticCoupling*beta_s*A_s_interface/r_s; ...
            filmThermoelasticCoupling*q*(A_f_interface+B_f_interface)/r_f- ...
                shearRatio*substrateThermoelasticCoupling*q*A_s_interface/r_s];
        elasticCoefficients = elasticSolver\elasticForcing;

        temperaturePositive(frequencyIndex) = A_f+B_f;
        displacementPositive(frequencyIndex) = ...
            filmThermoelasticCoupling*beta_f*(B_f-A_f)/r_f+ ...
            elasticCoefficients(1)+elasticCoefficients(3);
    end

%% Reconstruct the causal time-domain responses
    temperatureSpectrum = [conj(fliplr(temperaturePositive)), ...
        temperaturePositive];
    displacementSpectrum = [conj(fliplr(displacementPositive)), ...
        displacementPositive];
    temperature = real(inverseFourierPiecewiseLinear( ...
        omega,temperatureSpectrum,t))/(2*pi);
    displacement = real(inverseFourierPiecewiseLinear( ...
        omega,displacementSpectrum,t))/(2*pi);
end

function timeSignal = inverseFourierPiecewiseLinear(omega,spectrum,t)
%INVERSEFOURIERPIECEWISELINEAR Integrate a linearly interpolated spectrum.
    ...
    for timeIndex = 1:numel(t)
        ...
        integralLinear(~useSeries) = ...
            phase.*(interval/(1i*currentTime)+1/currentTime^2)- ...
            1/currentTime^2;

        interval = deltaOmega(useSeries);
        integralConstant(useSeries) = interval+ ...
            1i*currentTime*interval.^2/2-currentTime^2*interval.^3/6;
        integralLinear(useSeries) = interval.^2/2+ ...
            1i*currentTime*interval.^3/3-currentTime^2*interval.^4/8;

        timeSignal(timeIndex) = sum( ...
            exp(1i*currentTime*omega(1:end-1)).* ...
            (spectrum(1:end-1).*integralConstant+ ...
            spectralSlope.*integralLinear));
    end
end
